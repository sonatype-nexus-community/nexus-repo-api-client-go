/*
Sonatype Nexus Repository Manager

This documents the available APIs into [Sonatype Nexus Repository Manager](https://www.sonatype.com/products/sonatype-nexus-repository) as of version 3.94.0-12.

API version: 3.94.0-12
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v3

import (
	"encoding/json"
)

// checks if the CapabilityTypeDTO type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CapabilityTypeDTO{}

// CapabilityTypeDTO CapabilityType
type CapabilityTypeDTO struct {
	// Description of the capability type
	About *string `json:"about,omitempty"`
	// Form fields configuration for this capability type
	FormFields []FormFieldDTO `json:"formFields,omitempty"`
	// Capability type identifier
	Id *string `json:"id,omitempty"`
	// Display name of the capability type
	Name *string `json:"name,omitempty"`
}

// NewCapabilityTypeDTO instantiates a new CapabilityTypeDTO object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCapabilityTypeDTO() *CapabilityTypeDTO {
	this := CapabilityTypeDTO{}
	return &this
}

// NewCapabilityTypeDTOWithDefaults instantiates a new CapabilityTypeDTO object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCapabilityTypeDTOWithDefaults() *CapabilityTypeDTO {
	this := CapabilityTypeDTO{}
	return &this
}

// GetAbout returns the About field value if set, zero value otherwise.
func (o *CapabilityTypeDTO) GetAbout() string {
	if o == nil || IsNil(o.About) {
		var ret string
		return ret
	}
	return *o.About
}

// GetAboutOk returns a tuple with the About field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapabilityTypeDTO) GetAboutOk() (*string, bool) {
	if o == nil || IsNil(o.About) {
		return nil, false
	}
	return o.About, true
}

// HasAbout returns a boolean if a field has been set.
func (o *CapabilityTypeDTO) HasAbout() bool {
	if o != nil && !IsNil(o.About) {
		return true
	}

	return false
}

// SetAbout gets a reference to the given string and assigns it to the About field.
func (o *CapabilityTypeDTO) SetAbout(v string) {
	o.About = &v
}

// GetFormFields returns the FormFields field value if set, zero value otherwise.
func (o *CapabilityTypeDTO) GetFormFields() []FormFieldDTO {
	if o == nil || IsNil(o.FormFields) {
		var ret []FormFieldDTO
		return ret
	}
	return o.FormFields
}

// GetFormFieldsOk returns a tuple with the FormFields field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapabilityTypeDTO) GetFormFieldsOk() ([]FormFieldDTO, bool) {
	if o == nil || IsNil(o.FormFields) {
		return nil, false
	}
	return o.FormFields, true
}

// HasFormFields returns a boolean if a field has been set.
func (o *CapabilityTypeDTO) HasFormFields() bool {
	if o != nil && !IsNil(o.FormFields) {
		return true
	}

	return false
}

// SetFormFields gets a reference to the given []FormFieldDTO and assigns it to the FormFields field.
func (o *CapabilityTypeDTO) SetFormFields(v []FormFieldDTO) {
	o.FormFields = v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *CapabilityTypeDTO) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapabilityTypeDTO) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *CapabilityTypeDTO) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *CapabilityTypeDTO) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *CapabilityTypeDTO) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CapabilityTypeDTO) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *CapabilityTypeDTO) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *CapabilityTypeDTO) SetName(v string) {
	o.Name = &v
}

func (o CapabilityTypeDTO) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CapabilityTypeDTO) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.About) {
		toSerialize["about"] = o.About
	}
	if !IsNil(o.FormFields) {
		toSerialize["formFields"] = o.FormFields
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	return toSerialize, nil
}

type NullableCapabilityTypeDTO struct {
	value *CapabilityTypeDTO
	isSet bool
}

func (v NullableCapabilityTypeDTO) Get() *CapabilityTypeDTO {
	return v.value
}

func (v *NullableCapabilityTypeDTO) Set(val *CapabilityTypeDTO) {
	v.value = val
	v.isSet = true
}

func (v NullableCapabilityTypeDTO) IsSet() bool {
	return v.isSet
}

func (v *NullableCapabilityTypeDTO) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCapabilityTypeDTO(val *CapabilityTypeDTO) *NullableCapabilityTypeDTO {
	return &NullableCapabilityTypeDTO{value: val, isSet: true}
}

func (v NullableCapabilityTypeDTO) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCapabilityTypeDTO) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


